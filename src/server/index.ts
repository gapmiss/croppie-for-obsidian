// from: https://github.com/elias-sundqvist/obsidian-static-file-server
import http from 'http';
import { ALLOWED_HEADERS, mimeType } from '../settings/constants';
import url from 'url';
...
const exp = (folder: string, port: string, plugin: CroppiePlugin) => {
    ...
    const server = http.createServer(async function (req:any, res:any) {
        ...
        try {
          ...
          // spaces in filename fix
          let decodedURI = decodeURI(parsedUrl.pathname);
          const fullPath = normalizePath(folder + decodedURI);
